  ...
  module Game
    module G18IL2
      class Game < Game::Base
        ...
        BLOCKING_LOGOS = [
          '/logos/18_il2/yellow_blocking.svg', '/logos/18_il2/green_blocking.svg',
          '/logos/18_il2/brown_blocking.svg', '/logos/18_il2/gray_blocking.svg'
        ].freeze
        ...
        def concession_round
          G18IL2::Round::Auction.new(self, [
             G18IL2::Step::SelectionAuction,
          ])
        end

        def stock_round
          G18IL2::Round::Stock.new(self, [
            G18IL2::Step::HomeToken,
            G18IL2::Step::BuyNewTokens,
            G18IL2::Step::BaseBuySellParShares,
          ])
        end

        def operating_round(round_num)
          if intro_game?
            G18IL2::Round::Operating.new(self, [
              Engine::Step::Exchange,
              G18IL2::Step::SpecialTrack,
              Engine::Step::SpecialToken,
              Engine::Step::HomeToken,
              G18IL2::Step::ExchangeChoiceCorp,
              G18IL2::Step::ExchangeChoicePlayer,
              G18IL2::Step::Merge,
              Engine::Step::DiscardTrain,
              G18IL2::Step::Conversion,
              ...
              G18IL2::Step::CorporateSellShares,
              G18IL2::Step::Route,
              G18IL2::Step::Dividend,
              G18IL2::Step::SpecialBuyTrain,
              G18IL2::Step::BuyTrain,
              [G18IL2::Step::BuyCompany, { blocks: true }],
            ], round_num: round_num)
          else
            G18IL2::Round::Operating.new(self, [
              Engine::Step::Exchange,
              G18IL2::Step::SpecialTrack,
              G18IL2::Step::SpecialToken,
              Engine::Step::HomeToken,
              G18IL2::Step::ObsoleteTrain,
              G18IL2::Step::ExchangeChoiceCorp,
              G18IL2::Step::ExchangeChoicePlayer,
              G18IL2::Step::Merge,
              Engine::Step::DiscardTrain,
              ...
              G18IL2::Step::BorrowTrain,
              G18IL2::Step::CorporateSellShares,
              G18IL2::Step::BuyTrainBeforeRunRoute,
              G18IL2::Step::Route,
              G18IL2::Step::Dividend,
              G18IL2::Step::SpecialBuyTrain,
              G18IL2::Step::BuyTrain,
        ...
        def company_header(company)
          return 'CONCESSION LOT' if big_lots_variant? && @turn == 1
        ...
        def setup_preround
          ...
          setup_big_lots if big_lots_variant?
        ...
        def big_lots_variant?
          optional_rules&.include?(:big_lots_variant) && two_player?
        end
        ...
        def make_big_lot_company(idx)
          names = list_with_and(@big_lots[idx].map(&:name))
          ...
          Engine::Company.new(
            ...
            text_color: 'white',
            abilities: [],
            meta: { type: :big_lot, lot_index: idx },
          )
        end
        ...
        def all_bundles_for_corporation(share_holder, corporation, shares: nil)
          ...
          if !intro_game? && corporation == share_premium.owner &&
            @round.steps.find do |step|
              step.instance_of?(G18IL2::Step::SpecialIssueShares)
            end&.active?
          ...
          # halves the value of corporate-held shares if EMRing
                  ...
                  step.instance_of?(G18IL2::Step::CorporateSellShares)
                end&.active? &&
            !@round.steps.find do |step|
               step.instance_of?(G18IL2::Step::IssueShares)
             end&.active? &&
        ...
        def init_stock_market
          stock_market = G18IL2::StockMarket.new(self.class::MARKET, [], zigzag: :flip)
        ...
        def pay_fwc_bonus!(routes)
          ...
          @log << "#{frink_walker_co.owner.name} receives $20 from the bank (Frink, Walker, & Co.)"
        end
        ...
        def place_home_blocking_token(corporation)
          ...
          cities.each do |city|
            ...
            # token = Token.new(corporation, price: 0, logo: "/logos/18_il2/#{corporation.name}.svg",
            #                                simple_logo: "/logos/18_il2/#{corporation.name}.alt.svg", type: :blocking)
            token = Token.new(corporation, price: 0, logo: "/logos/18_il2/#{corporation.name}.alt.svg",
                                           simple_logo: "/logos/18_il2/#{corporation.name}.alt.svg", type: :blocking)
            token.status = :flipped
        ...
        def redeemable_shares(entity)
          return [] unless entity.corporation?
          return [] unless @round.steps.find { |step| step.is_a?(G18IL2::Step::BaseBuySellParShares) }.active?
        ...
        def mergeable_corporations
          ...
          4.times do |i|
            ...
            ic_line_corporations << corp if corp && corp != ic && !@closed_corporations.include?(corp) && corp.ipoed
          end
        ...
        def merge_corporation_part_one(corporation = nil)
          ...
          # Calculate total refund for player-owned shares
          @players.each do |entity|
            entity.shares_of(corporation).dup.each do |share|
              next if !share || corporation == entity || share.president

              multiplier = corporation.president?(entity) ? 0.5 : 1.0
              @total_refund += refund * multiplier
            end
          end
          # Adds refund for market shares
          @total_refund += (corporation.num_market_shares * corporation.share_price.price)
        ...
        def merge_corporation_part_two
          ...
          # Corporation sells IC shares to the open market
          shares = corporation.shares_of(ic)
          if shares&.any?
            bundle = ShareBundle.new(shares)
            ic_sale = bundle.num_shares * ic.share_price.price
            @log << "#{corporation.name} sells "\
                    "#{bundle.num_shares} share#{'s' unless bundle.num_shares == 1} of #{ic.name} for #{format_currency(ic_sale)}"
            @share_pool.transfer_shares(bundle, corporation)
          ...
          if corporation.trains.any?
            ...
            @log << "#{ic.name} receives #{trains_transferred.one? ? 'a train' : 'trains'} "\
                    "from #{corporation.name}: #{list_with_and(trains_transferred)}"
          end
