    ...
    module G18IL
      module Round
        class Stock < Engine::Round::Stock
          ...
          def help
            return ["Note: Treasury shares of IC cannot be purchased in stock round"] if @ic_formation_triggered && !@game.ic.num_ipo_shares.zero?
            []
          end
          ...
          def sold_out?(corporation)
            corporation.total_shares > 2 && (corporation.player_share_holders.values.sum + corporation.corporate_share_holders.values.sum) >= 100
          end
        end
      end
    end
